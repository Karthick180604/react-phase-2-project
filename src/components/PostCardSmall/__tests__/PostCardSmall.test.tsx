...
describe("PostCardSmall Component", () => {
  ...
  describe("Title Truncation Logic", () => {
    it("displays full title when length is exactly 60 characters", () => {
      const exactTitle = "A".repeat(60); // Exactly 60 characters
    ...
    it("displays full title when length is less than 60 characters", () => {
      const shortTitle = "Short title"; // Less than 60 characters
    ...
    it("truncates title when length is greater than 60 characters", () => {
      const longTitle = "A".repeat(65); // 65 characters, should be truncated
      ...
      expect(titleElement.textContent).toHaveLength(63); // 60 + "..."
    });

    it("truncates title at exactly 61 characters", () => {
      const title61Chars = "A".repeat(61); // 61 characters
      const expectedTruncated = "A".repeat(60) + "...";
  ...
  describe("Body Truncation Logic", () => {
    it("displays full body when length is exactly 100 characters followed by space", () => {
      const exactBody = "A".repeat(100); // Exactly 100 characters
      renderComponent({ body: exactBody });

      const bodyElement = screen.getByTestId("postcard-body");
      // Should show full body + space + "Read more"
    ...
    it("displays full body when length is less than 100 characters", () => {
      const shortBody = "Short body content"; // Less than 100 characters
    ...
    it("truncates body when length is greater than 100 characters", () => {
      const longBody = "A".repeat(105); // 105 characters, should be truncated
      const expectedTruncated = "A".repeat(100);
      renderComponent({ body: longBody });

      const bodyElement = screen.getByTestId("postcard-body");
      expect(bodyElement.textContent).toContain(expectedTruncated + "... Read more");
      expect(bodyElement.textContent).not.toContain("A".repeat(105));
    });

    it("truncates body at exactly 101 characters", () => {
      const body101Chars = "A".repeat(101); // 101 characters
      const expectedTruncated = "A".repeat(100);
  ...
  describe("Edge Cases and Boundary Conditions", () => {
    ...
    it("handles newlines and whitespace in title", () => {
      ...
      const titleElement = screen.getByTestId("postcard-title");
      // HTML normalizes whitespace - newlines and multiple spaces become single spaces
      expect(titleElement).toHaveTextContent("Title with newlines and tabs and spaces");
    });
  });

  describe("Combined Truncation Scenarios", () => {
    it("handles both title and body needing truncation", () => {
      const longTitle = "A".repeat(70); // Needs truncation
      const longBody = "B".repeat(110); // Needs truncation
      ...
      expect(bodyElement.textContent).toContain("B".repeat(100) + "... Read more");
    });
  ...
  describe("Accessibility", () => {
    ...
    it("maintains text hierarchy with proper Typography variants", () => {
      ...
      // Material-UI Typography components should be rendered
      expect(titleElement.closest('[class*="MuiTypography"]')).toBeInTheDocument();
      expect(bodyElement.closest('[class*="MuiTypography"]')).toBeInTheDocument();
    });
  });
  ...
  describe("Props Handling", () => {
    it("handles all required props correctly", () => {
      ...
      expect(screen.getByTestId("postcard-title")).toHaveTextContent("Custom Title");
      expect(screen.getByTestId("postcard-body")).toHaveTextContent("Custom Body Content");
    ...
    it("handles prop updates correctly", () => {
      ...
      expect(screen.getByTestId("postcard-title")).toHaveTextContent("Original Title");
      ...
      expect(screen.getByTestId("postcard-title")).toHaveTextContent("Updated Title");
    });
  });
});