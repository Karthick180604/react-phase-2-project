...
jest.mock('../../../components/PostDialog/PostDialog', () => {
  return function MockPostDialog({ open, onClose, post, onLikeHandler, onDislikeHandler, like, dislike }: any) {
    if (!open) return null;
    return (
...
jest.mock('../../../components/PostCardSmallSkeleton/PostCardSmallSkeleton', () => {
...
const userReducer = (state = { uploadedPosts: [], likedPostId: [], dislikePostId: [] }, action: any) => {
  switch (action.type) {
    case 'LIKE_POST':
      return { ...state, likedPostId: [...state.likedPostId, action.payload] };
    case 'REMOVE_LIKE_POST':
      return { ...state, likedPostId: state.likedPostId.filter(id => id !== action.payload) };
    case 'DISLIKE_POST':
      return { ...state, dislikePostId: [...state.dislikePostId, action.payload] };
    case 'REMOVE_DISLIKE_POST':
      return { ...state, dislikePostId: state.dislikePostId.filter(id => id !== action.payload) };
    default:
      return state;
  }
};
...
const createMockStore = (initialState = {}): Store<any, AnyAction> => {
  ...
  // Create a simple mock store without middleware to avoid conflicts
...
describe('SearchPosts Component', () => {
  beforeEach(() => {
    ...
    // Mock fetchPosts to return a thunk that resolves with payload
    mockedPostsActions.fetchPosts.mockImplementation((page: number, limit: number) => {
      return (dispatch: any) => {
    ...
    mockedUserActions.likePost.mockReturnValue({ type: 'LIKE_POST', payload: 1 });
    mockedUserActions.removeLikePost.mockReturnValue({ type: 'REMOVE_LIKE_POST', payload: 1 });
    mockedUserActions.dislikePost.mockReturnValue({ type: 'DISLIKE_POST', payload: 1 });
    mockedUserActions.removeDislikePost.mockReturnValue({ type: 'REMOVE_DISLIKE_POST', payload: 1 });
    mockedApiCalls.getAllPostTags.mockResolvedValue({ data: mockTags });
    mockedApiCalls.getSearchedPosts.mockResolvedValue({ data: { posts: mockPosts } });
  });
  ...
  describe('Initial Render', () => {
    it('should render the component with correct title', () => {
      ...
      expect(screen.getByTestId('page-title')).toHaveTextContent('Search Posts');
    });
  ...
  describe('Search Functionality', () => {
    it('should handle search input changes', async () => {
      ...
      await waitFor(() => {
        expect(mockedApiCalls.getSearchedPosts).toHaveBeenCalledWith('test search');
      });
    });
    ...
    it('should handle search API errors gracefully', async () => {
      ...
      mockedApiCalls.getSearchedPosts.mockRejectedValue(new Error('Search failed'));
  ...
  describe('Tag Filtering', () => {
    it('should render tag options', async () => {
      ...
      // Open the select to check options
    ...
    it('should filter posts by selected tag', async () => {
      ...
      // Wait for options to appear and click React option
      await waitFor(() => {
      ...
      // Should filter posts with 'react' tag
      expect(screen.getByTestId('post-card-1')).toBeInTheDocument();
    ...
    it('should reset pagination when "All" tag is selected', async () => {
      ...
      // Wait for options and click All
      await waitFor(() => {
  ...
  describe('Post Dialog', () => {
    ...
    it('should handle remove like action when post is already liked', async () => {
      ...
      // Click like button (should remove like)
      const likeButton = screen.getByTestId('like-button');
      await user.click(likeButton);
    ...
    it('should handle remove dislike action when post is already disliked', async () => {
      ...
      // Click dislike button (should remove dislike)
      const dislikeButton = screen.getByTestId('dislike-button');
      await user.click(dislikeButton);
  ...
  describe('Infinite Scrolling', () => {
    ...
    it('should not trigger pagination when searching', () => {
      ...
      // Should not increment page when searching
  ...
  describe('Error Handling', () => {
    ...
    it('should handle tag fetching errors gracefully', async () => {
      ...
      mockedApiCalls.getAllPostTags.mockRejectedValue(new Error('Tags fetch failed'));
  ...
  describe('Component Cleanup', () => {
    it('should disconnect intersection observer on unmount', () => {
      ...
      // Note: The actual cleanup happens in the useCallback dependency change
      // This test ensures the observer is properly created
      expect(mockIntersectionObserver).toHaveBeenCalled();
    });
  });
  ...
  describe('Pagination', () => {
    it('should handle pagination correctly when more posts are available', async () => {
      const mockFetchPosts = jest.fn().mockReturnValue(
        (dispatch: any) => {
          dispatch({ type: 'FETCH_POSTS_SUCCESS', payload: mockPosts });
          return Promise.resolve({ payload: mockPosts });
        }
    ...
    it('should set hasMore to false when less posts than limit are returned', async () => {
      const shortPostList = [mockPosts[0]]; // Less than limit of 15
      const mockFetchPosts = jest.fn().mockReturnValue(
        (dispatch: any) => {
  ...
  describe('State Management', () => {
    ...
    it('should combine uploaded posts with fetched posts', () => {
      ...
      // Should have both uploaded and fetched posts
      expect(screen.getByTestId('post-card-999')).toBeInTheDocument();
      expect(screen.getByTestId('post-card-1')).toBeInTheDocument();
      expect(screen.getByTestId('post-card-2')).toBeInTheDocument();
    });
