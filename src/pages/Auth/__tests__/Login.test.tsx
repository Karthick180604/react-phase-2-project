...
describe("Login Component", () => {
  ...
  beforeEach(() => {
    store = mockStore({
      user: { email: "", password: "" }, // initial state
    });
    jest.clearAllMocks();
  });
 ...
 it("enables login button when fields are filled", () => {
  ...
  fireEvent.change(within(screen.getByTestId("email-input")).getByRole("textbox"), {
    target: { value: "john@example.com" },
  });
  fireEvent.change(within(screen.getByTestId("password-input")).getByLabelText("Password"), {
    target: { value: "secret123" },
  });
