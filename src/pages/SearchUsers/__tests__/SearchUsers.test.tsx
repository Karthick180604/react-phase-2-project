...
describe("SearchUsers Component", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockedApiCalls.getAllUsers.mockResolvedValue({
      data: { users: mockUsers },
    });
    mockedApiCalls.getSearchedUsers.mockResolvedValue({
      data: { users: mockUsers },
    });
  });
  ...
  describe("Initial Render", () => {
    it("should render the component with correct title", () => {
      ...
      expect(screen.getByTestId("search-users-title")).toHaveTextContent(
        "Explore Users",
      );
    });
  ...
  describe("Search Functionality", () => {
    ...
    it("should search for users and display results", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("user-card-component-1")).toBeInTheDocument();
        expect(
          screen.queryByTestId("user-card-component-2"),
        ).not.toBeInTheDocument();
        expect(
          screen.queryByTestId("user-card-component-3"),
        ).not.toBeInTheDocument();
      });
    });
  ...
  describe("Error Handling", () => {
    it("should render API error component when hasApiError is true", () => {
      ...
      expect(
        screen.queryByTestId("search-users-container"),
      ).not.toBeInTheDocument();
    });

    it("should handle getAllUsers API error gracefully", async () => {
      const consoleSpy = jest.spyOn(console, "log").mockImplementation();
      mockedApiCalls.getAllUsers.mockRejectedValue(
    ...
    it("should handle getSearchedUsers API error gracefully", async () => {
      const consoleSpy = jest.spyOn(console, "log").mockImplementation();
      mockedApiCalls.getSearchedUsers.mockRejectedValue(
  ...
  describe("Component State Management", () => {
    it("should initialize with empty user list and search term", () => {
      mockedApiCalls.getAllUsers.mockImplementation(
        () => new Promise(() => {}),
      );
    ...
    it("should maintain search term state during searches", async () => {
      ...
      await waitFor(() => {
        expect(mockedApiCalls.getSearchedUsers).toHaveBeenCalledWith(
          "John Doe",
        );
      });
  ...
  describe("Async Operations", () => {
    ...
    it("should handle async search correctly", async () => {
      const user = userEvent.setup();
      const searchResult = [mockUsers[0]];
      const mockSearchPromise = Promise.resolve({
        data: { users: searchResult },
      });
